...
write_network_configuration () {
	...
	cat <<-EOF >> /etc/network/interfaces
		...
		iface $SETUP_PVEL3_MGMT_PORT inet static
		    ...
		    # クラスター作成時は本ファイルに IP アドレスの設定を記述しておく必要がある
		    # この記述が無いままクラスターを作成しようとしても、ネットワークの一覧に候補が表示されない
		    address $SETUP_PVE_MGMT_CIDR
		    # クラスター作成後はスクリプトとネットワーク設定ファイルに IP アドレスの設定を任せられる
		    # ネットワークの定義をネットワーク設定ファイルにまとめたい場合はスクリプトを利用する
