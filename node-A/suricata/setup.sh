...
# 引数 : WAN 側ネットワーク
SETUP_WAN_NET="192.168.0.0/24"
# 引数 : 使用する Suricata のルールの名前
SETUP_ENABLE_SURICATA_RULE_NAME="et/open"
...
install_suricata () {
	...
	[ -f /etc/apt/sources.list.d/ubuntu.sources ] && conf=/etc/apt/sources.list.d/ubuntu.sources || conf=/etc/apt/sources.list
	# archive.ubuntu.com もしくは us.archive.ubuntu.com を jp.archive.ubuntu.com へ置換する
	sed -i.org -r 's@(archive|us\.archive)\.ubuntu\.com@jp.archive.ubuntu.com@g' "$conf"
	apt-get update && apt-get install suricata -y
}

write_etc_suricata_suricata_yaml () {
	cp /etc/suricata/suricata.yaml /etc/suricata/suricata.yaml.org
...
run_suricata_update () {
	...
	suricata-update enable-source $SETUP_ENABLE_SURICATA_RULE_NAME
